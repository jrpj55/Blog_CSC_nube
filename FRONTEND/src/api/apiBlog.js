...
export class apiBlog {
  ...
  async createPost(accessToken, data) {
    try {
      ...
      if (data.file) {
        formData.append('miniature_blog', data.file);
      }
  ...
  async getPosts(page = 1, limit = 10) {
    try {
      ...
      const url = `${this.baseApi}/${ENV.API_ROUTES.POST_OBT}?${pageFilter}&${limitFilter}`;
  ...
  async updatePost(accessToken, idPost, data) {
    try {
      ...
      const url = `${this.baseApi}/${ENV.API_ROUTES.POST_A_E}/${idPost}`;
      const params = {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${accessToken}`,
